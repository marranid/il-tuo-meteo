...
router.get('/meteo', async (req,res) => {
  ...
  if(lat || lon && aq){
    ...
    if(lat==undefined || lon==undefined || aq==undefined){
      //onecall 1.0 api
      const weatherUrl = `https://api.openweathermap.org/data/2.5/onecall?lat=45.8084878&lon=9.5768675&units=metric&exclude=minutely,hourly&appid=${process.env.API_KEY}`
      try{
        await fetch(weatherUrl)
          .then(res => res.json())
          .then(data => {
